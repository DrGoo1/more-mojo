...
class MLARAgent:
    ...
    def run():
        ...
        # Import the actual MLARAgent implementation
        try:
            sys.path.append(str(pathlib.Path(__file__).parent))
            from mlar_agent import MLARAgent as MLARImplementation
            changed = MLARImplementation.run()
            if changed:
                stage_all()
            ...
            print(f"MLARAgent: Error during integration - {e}")
            return False
...
def main():
    ...
    decisions = {
        ...
        "MLARAgent":    MLARAgent.wants(app_log + plugin_log),  # Check in both logs for MLAR keywords
    }
    ...
    if decisions["MLARAgent"]:    MLARAgent.run();    summary.append("- Ran MLARAgent");    acted=True
    ...
    # MLAR excerpt if needed
    if decisions["MLARAgent"]:
        mlar_excerpt = first_match_excerpt(app_log + plugin_log, r"(MLAR integration needed|mlar_enabled|MLARIntegration|More Like The Record)")
        if mlar_excerpt:
            summary += ["", "## MLAR Integration Excerpt", "```", mlar_excerpt, "```"]
