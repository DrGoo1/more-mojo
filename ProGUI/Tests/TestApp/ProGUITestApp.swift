...
private func openStandaloneDemoWindow() {
    if let w = proguiStandaloneWindow, w.isVisible { return }
    ...
    container.layer?.backgroundColor = NSColor.systemRed.withAlphaComponent(0.15).cgColor
    ...
    let border = NSBox(frame: NSRect(x: 8, y: 8, width: container.bounds.width - 16, height: container.bounds.height - 16))
    border.boxType = .custom
    border.borderType = .lineBorder
    border.borderWidth = 2.0
    border.borderColor = NSColor.systemBlue.withAlphaComponent(0.6)
    border.fillColor = .clear
    border.autoresizingMask = [.width, .height]
    container.addSubview(border, positioned: .below, relativeTo: header)
    ...
    juceView.layer?.backgroundColor = NSColor.systemGreen.withAlphaComponent(0.15).cgColor
    let juceBorder = NSBox(frame: NSRect(x: 12, y: 12, width: container.bounds.width - 24, height: container.bounds.height - 24))
    juceBorder.boxType = .custom
    juceBorder.borderType = .lineBorder
    juceBorder.borderWidth = 2.0
    juceBorder.borderColor = NSColor.systemGreen.withAlphaComponent(0.6)
    juceBorder.fillColor = .clear
    juceBorder.autoresizingMask = [.width, .height]
    juceView.addSubview(juceBorder)
    let inside = NSTextField(labelWithString: "INSIDE JUCE CONTAINER")
    ...
    bridge.setOverlayVisible(false)
    ...
    // Show an alert so we 100% know the current build executed this code
    let alert = NSAlert()
    alert.messageText = "Standalone Demo"
    alert.informativeText = "This alert proves the latest build ran openStandaloneDemoWindow()"
