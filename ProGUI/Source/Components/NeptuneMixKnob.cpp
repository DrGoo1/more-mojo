#include "NeptuneMixKnob.h"

NeptuneMixKnob::NeptuneMixKnob()
{
    loadNeptuneFrames();
}

void NeptuneMixKnob::setValue(float newValue)
{
    currentValue = juce::jlimit(0.0f, 1.0f, newValue);
    repaint();
}

void NeptuneMixKnob::paint(juce::Graphics& g)
{
    ...
    else
    {
        ...
        g.drawText("MIX", getLocalBounds(), juce::Justification::centred);
    }
}

void NeptuneMixKnob::mouseDown(const juce::MouseEvent& event)
{
    lastMousePos = event.getPosition();
}

void NeptuneMixKnob::mouseDrag(const juce::MouseEvent& event)
{
...
void NeptuneMixKnob::loadNeptuneFrames()
{
    ...
    juce::Logger::writeToLog("Neptune Mix Knob: Loaded " + juce::String(neptuneFrames.size()) + " frames");
}
