...
@objc public class ProGUIBridge: NSObject {
    ...
    private weak var debugLabel: NSTextField?
    ...
    public init(type: ProGUIComponentType, frame: CGRect) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.05) { [weak self] in
            ...
            self.installFrameChangeObserver(logger: logger)
    ...
    private func installFrameChangeObserver(logger: Logger) {
        view.postsFrameChangedNotifications = true
        frameObserver = NotificationCenter.default.addObserver(forName: NSView.frameDidChangeNotification, object: view, queue: .main) { [weak self] _ in
            guard let self = self else { return }
            let b = self.view.bounds
            if let component = self.componentPtr {
                ProGUI_ResizeComponent(component, Float(b.width), Float(b.height))
            }
            for sub in self.view.subviews { sub.frame = b }
            self.view.needsLayout = true
            self.view.layoutSubtreeIfNeeded()
            self.updateDebugOverlay()
            NSLog("[Bridge] Frame change -> resized to \(Int(b.width))x\(Int(b.height)); subviews=\(self.view.subviews.count)")
            logger.info("[ProGUIBridge] Frame change resized to: \(Int(b.width))x\(Int(b.height))")
        }
    }

    private func addDebugOverlay() {
        let label = NSTextField(labelWithString: "[Bridge] attached")
        label.textColor = .systemYellow
        label.font = .boldSystemFont(ofSize: 11)
        label.backgroundColor = .clear
        label.alignment = .left
        label.frame = NSRect(x: 6, y: view.bounds.height - 18, width: view.bounds.width - 12, height: 14)
    ...
    private func updateDebugOverlay() {
        ...
        label.stringValue = "[Bridge] attached — \(Int(sz.width))x\(Int(sz.height)) subviews=\(view.subviews.count)"
    }
