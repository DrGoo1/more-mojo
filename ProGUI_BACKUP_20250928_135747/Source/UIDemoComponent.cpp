...
UIDemoComponent::UIDemoComponent()
{
    ...
    // Create Neptune process knobs (8 processes)
    for (int i = 0; i < 8; ++i)
    {
        processKnobs[i] = std::make_unique<NeptuneKnob>();
        processKnobs[i]->setValue(0.5f + i * 0.05f); // Different values for each
        addAndMakeVisible(*processKnobs[i]);
    }
...
{{ ... }}
    ...
    // Fill center area with Pro Master controls
    {
        ...
        if (mixKnobArea.getHeight() > 0)
        {
            ...
            auto leftLabel = labelRect.removeFromLeft(labelRect.getWidth() / 3);
            auto rightLabel = labelRect.removeFromRight(labelRect.getWidth() / 2);
            auto centerLabel = labelRect;
            
            g.setColour(juce::Colours::white);
            g.setFont(juce::Font("Arial", 10.0f, juce::Font::bold));
            g.drawText("ORIGINAL", leftLabel, juce::Justification::centred);
            g.drawText("MIX", centerLabel, juce::Justification::centred);
            g.drawText("PROCESSED", rightLabel, juce::Justification::centred);
        }
        ...
        if (remainingArea.getHeight() > 100)
        {
            ...
            // Draw process labels and buttons (Neptune knobs are now actual components)
    ...
    // DEBUG: Draw blue rectangles around knob positions - MAKE THEM VERY VISIBLE AND DRAGGABLE
    g.setColour(juce::Colours::blue);
    int debugCount = 0;
    for (int i = 0; i < 8; ++i)
    {
        if (!knobDebugRects[i].isEmpty())
        {
            // Draw thick blue border
            g.drawRect(knobDebugRects[i].toFloat(), 4.0f);
            // Fill with semi-transparent blue for better visibility
            g.setColour(juce::Colours::blue.withAlpha(0.3f));
            g.fillRect(knobDebugRects[i].toFloat());
            g.setColour(juce::Colours::blue);
            // Draw knob number in center
            g.setFont(juce::Font(12.0f, juce::Font::bold));
            g.drawText(juce::String(i), knobDebugRects[i], juce::Justification::centred);
            debugCount++;
        }
    }
    std::cout << "Drawing " << debugCount << " blue debug rectangles" << std::endl;
...
void UIDemoComponent::resized()
{
    ...
    if (workArea.getHeight() > 100)
    {
        ...
        // OLD POSITIONING CODE DISABLED - now using forceNeptuneKnobPositioning() instead
        /*
        for (int i = 0; i < 8; ++i)
        {
            if (processKnobs[i])
            {
                // Position Neptune knobs in Professional Mojo Controls area
                processKnobs[i]->setVisible(true);
                
                // Calculate proper position based on process layout
                int col = i % 2;  // 0 = left column, 1 = right column
                int row = i / 2;  // 0-3 for the 4 rows
                
                int knobX, knobY;
                if (col == 0) // Left column - more to the left and down
                {
                    knobX = processArea.getX() + 15;  // Far left
                    knobY = processArea.getY() + 50 + row * 120;  // Down from top, spaced by 120px
                }
                else // Right column - slightly to the right and down  
                {
                    knobX = processArea.getX() + processArea.getWidth()/2 + 25;  // Center-right
                    knobY = processArea.getY() + 50 + row * 120;  // Same vertical spacing
                }
                
                auto knobRect = juce::Rectangle<int>(knobX, knobY, 55, 55);
                processKnobs[i]->setBounds(knobRect);
                
                // Connect knob value to color bar level
                processKnobs[i]->onValueChanged = [this, i](float value) {
                    processLevels[i] = value;
                    repaint();
                };
            }
        }
    ...
    // Position ISP knob directly here to avoid override issues
    if (processKnobs[0])
    {
        // Position knob INSIDE the Professional Mojo Controls border, to the left of ISP section
        // Need to move knob even higher UP to center with ISP section
        int knobX = 120;  // Left of "IMAGE SIGNAL PROCESSING" text
        int knobY = 315;  // Move much higher UP to center with ISP section
        
        auto knobRect = juce::Rectangle<int>(knobX, knobY, 45, 45);
        processKnobs[0]->setBounds(knobRect);
        processKnobs[0]->setVisible(true);
        
        juce::Logger::writeToLog("ISP knob positioned even higher UP at: " + knobRect.toString());
    }
    
    // Force repaint to ensure visual changes appear
    this->repaint();
    
    // FORCE Neptune knob positioning - call this at the very end to override any other positioning
    forceNeptuneKnobPositioning(); // RE-ENABLED to ensure knob appears
...
void UIDemoComponent::forceNeptuneKnobPositioning()
{
    std::cout << "=== forceNeptuneKnobPositioning() CALLED ===" << std::endl;
    
    // Use EXACT same layout calculation as resized() function
    auto bounds = getLocalBounds();
    auto outer = bounds.reduced(8, 16);  // Match resized() function
    const int colW = 200; // Match resized() function
    const int gap = 8;    // Match resized() function
    auto leftCol  = outer.removeFromLeft(colW);
    outer.removeFromLeft(gap);
    auto rightCol = outer.removeFromRight(colW);
    outer.removeFromRight(gap);
    ...
    // Calculate work area same as resized()
    auto workArea = centerPanel.reduced(0, 2);
    int wfH = juce::jlimit(50, 70, workArea.getHeight() - 25);
    auto waveformArea  = workArea.removeFromTop(wfH);
    auto transportArea = workArea.removeFromTop(35);
    auto mixKnobArea = workArea.removeFromTop(100);
    workArea.removeFromTop(2);
    workArea = workArea.reduced(10, 0);
    
    // ISP knob positioning is now handled in the unified loop below
    
    if (workArea.getHeight() > 100)
    {
        auto processArea = workArea.reduced(8);
        
        // SIMPLIFIED APPROACH: Use hardcoded positions based on visual inspection
        // Each knob should be positioned to the left of its corresponding cyan slider
        
        // Hardcoded positions for each knob based on the 2x4 grid layout
        // Left column: ISP(0), JITTER(2), TRANSIENT(4), MLAR(6)  
        // Right column: SRC(1), ALIGN(3), DEESSER(5), TRANSFORMER(7)
        
        struct KnobPosition {
            int x, y;
        };
        
        // FINAL KNOB POSITIONS - Based on successful positioning
        KnobPosition knobPositions[8] = {
            // LEFT COLUMN - Positioned correctly with layout shift
            {160, 250},  // ISP (0) - Image Signal Processing
            {420, 250},  // SRC (1) - Sample Rate Conversion  
            {160, 315},  // JITTER (2) - Jitter & Accumulation
            {420, 315},  // ALIGN (3) - Phase Alignment
            {160, 380},  // TRANSIENT (4) - Transient Shaping
            {420, 380},  // DEESSER (5) - De-essing
            {160, 445},  // MLAR (6) - More Like A Record
            {420, 445}   // TRANSFORMER (7) - Transformer
        };
        ...
        for (int i = 0; i < 8; ++i)
        {
            if (processKnobs[i])
            {
                // Only set position if this knob hasn't been manually positioned
                if (!knobPositionsCustomized[i])
                {
                    auto knobRect = juce::Rectangle<int>(
                        knobPositions[i].x, 
                        knobPositions[i].y, 
                        35, 35  // Smaller size for easier positioning
                    );
                    
                    processKnobs[i]->setBounds(knobRect);
                    knobDebugRects[i] = knobRect;
                    
                    std::cout << "Process " << i << " knob at DEFAULT position: " << knobRect.getX() << "," << knobRect.getY() << std::endl;
                }
                else
                {
                    // Keep the manually positioned location
                    processKnobs[i]->setBounds(knobDebugRects[i]);
                    std::cout << "Process " << i << " knob at CUSTOM position: " << knobDebugRects[i].getX() << "," << knobDebugRects[i].getY() << std::endl;
                }
                
                processKnobs[i]->setVisible(true);
                processKnobs[i]->setInterceptsMouseClicks(false, false); // Disable mouse events so we can drag the debug rectangles
            }
        }
    }
}
...
{
    // Check if clicking on a knob debug rectangle for dragging
    auto clickPos = event.getPosition();
    draggedKnobIndex = -1;
    
    for (int i = 0; i < 8; ++i)
    {
        if (!knobDebugRects[i].isEmpty() && knobDebugRects[i].contains(clickPos))
        {
            draggedKnobIndex = i;
            dragOffset = clickPos - knobDebugRects[i].getTopLeft();
            std::cout << "Started dragging knob " << i << std::endl;
            return;
        }
    }
...
void UIDemoComponent::mouseDrag(const juce::MouseEvent& event)
{
    if (draggedKnobIndex >= 0)
    {
        auto newPos = event.getPosition() - dragOffset;
        knobDebugRects[draggedKnobIndex].setPosition(newPos);
        
        // Update the actual knob position
        if (processKnobs[draggedKnobIndex])
        {
            processKnobs[draggedKnobIndex]->setBounds(knobDebugRects[draggedKnobIndex]);
        }
...
void UIDemoComponent::mouseUp(const juce::MouseEvent& event)
{
    if (draggedKnobIndex >= 0)
    {
        auto rect = knobDebugRects[draggedKnobIndex];
        std::cout << "Knob " << draggedKnobIndex << " final position: {" << rect.getX() << ", " << rect.getY() << "}" << std::endl;
        
        // Mark this knob as manually positioned so it won't be reset on resize
        knobPositionsCustomized[draggedKnobIndex] = true;
