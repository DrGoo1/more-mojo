...
private:
    ...
    // Process knobs (8 processes, pure Neptune knobs)
    std::unique_ptr<class NeptuneKnob> processKnobs[8];
    ...
    juce::Rectangle<int> knobDebugRects[8]; // DEBUG: Store knob positions for visual debugging
    
    // Drag support for knob positioning
    int draggedKnobIndex = -1;
    juce::Point<int> dragOffset;
    bool knobPositionsCustomized[8] = {false}; // Track which knobs have been manually positioned
