...
public:
    struct Skin {
        ...
        std::vector<juce::Image> knobFrames;     // filmstrip frames for knob (big)
        std::vector<juce::Image> knobSmallFrames; // filmstrip frames for small knob
        std::vector<juce::Image> sliderVertFrames; // filmstrip frames for vertical slider
    ...
    bool loadFantasmaAt(const juce::String& folderPath) {
        ...
        if (knobBig.isDirectory())
        {
            ...
            if (! next.knobFrames.empty())
            {
                next.knobImage = next.knobFrames.front();
                loadedAny = true;
                logToFile("[SkinManager] Loaded Neptune knob_big: " + juce::String((int)next.knobFrames.size()) + " frames");
            }
        }
        
        // Load Neptune slider_ver
        auto sliderVer = f.getChildFile("Oneshots").getChildFile("slider_ver");
        if (sliderVer.isDirectory())
        {
            next.sliderVertFrames.clear();
            auto files = sliderVer.findChildFiles(juce::File::findFiles, false, "*.png");
            struct FileComparator {
                int compareElements(const juce::File& a, const juce::File& b) const {
                    return a.getFileName().compare(b.getFileName());
                }
            ...
            {
                auto img = juce::ImageFileFormat::loadFrom(file);
                if (img.isValid()) next.sliderVertFrames.push_back(img);
            }
            if (! next.sliderVertFrames.empty())
            {
                loadedAny = true;
                logToFile("[SkinManager] Loaded Neptune slider_ver: " + juce::String((int)next.sliderVertFrames.size()) + " frames");
            }
        }
        
        // Load Neptune knob_small
        auto knobSmall = f.getChildFile("Oneshots").getChildFile("knob_small");
        if (knobSmall.isDirectory())
        {
            next.knobSmallFrames.clear();
            auto files = knobSmall.findChildFiles(juce::File::findFiles, false, "*.png");
            struct FileComparator {
                int compareElements(const juce::File& a, const juce::File& b) const {
                    return a.getFileName().compare(b.getFileName());
                }
            ...
            {
                auto img = juce::ImageFileFormat::loadFrom(file);
                if (img.isValid()) next.knobSmallFrames.push_back(img);
            }
            if (! next.knobSmallFrames.empty())
            {
                loadedAny = true;
                logToFile("[SkinManager] Loaded Neptune knob_small: " + juce::String((int)next.knobSmallFrames.size()) + " frames");
            }
        }
        
        // Load Neptune buttons
        auto buttonBigOff = f.getParentDirectory().getChildFile("button_big_off.png");
        auto buttonBigOn = f.getParentDirectory().getChildFile("button_big_on.png");
        if (buttonBigOff.existsAsFile() && buttonBigOn.existsAsFile())
        {
            next.buttonFrames.clear();
            auto imgOff = juce::ImageFileFormat::loadFrom(buttonBigOff);
            auto imgOn = juce::ImageFileFormat::loadFrom(buttonBigOn);
            if (imgOff.isValid() && imgOn.isValid()) {
                next.buttonFrames.push_back(imgOff);
                next.buttonFrames.push_back(imgOn);
                loadedAny = true;
                logToFile("[SkinManager] Loaded Neptune buttons: 2 frames");
            }
        }
