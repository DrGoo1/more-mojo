...
{{ ... }}
    ...
    // Fill center area with Pro Master controls
    {
        ...
        workArea.removeFromTop(25);  // skip transport area (smaller to match resized())
        
        // Apply same margin reduction to prevent knob overlap
        workArea = workArea.reduced(8, 0); // smaller side margins
        
        // Mix knob above Professional Mojo Controls - much taller
        auto mixKnobArea = workArea.removeFromTop(120); // 120px height for much larger mix knob
        workArea.removeFromTop(2); // very minimal gap between knob and Mojo window
        
        if (mixKnobArea.getHeight() > 0)
        {
            // Center the mix knob in the available area
            auto knobSize = juce::jmin(mixKnobArea.getWidth() / 4, mixKnobArea.getHeight() - 20); // leave space for label
            auto knobRect = juce::Rectangle<int>(knobSize, knobSize).withCentre(mixKnobArea.getCentre());
            knobRect = knobRect.withY(mixKnobArea.getY() + 5); // slight offset from top
            
            // Draw Mix knob using Neptune big knob
            if (! skin.knobFrames.empty())
            {
                // Use 50% position for mix knob (equal blend)
                int frameIdx = (int)(skin.knobFrames.size() * 0.5f);
                frameIdx = juce::jlimit(0, (int)skin.knobFrames.size() - 1, frameIdx);
                auto img = skin.knobFrames[frameIdx];
                float scale = juce::jmin(knobRect.getWidth() / (float)img.getWidth(), 
                                       knobRect.getHeight() / (float)img.getHeight()) * 0.95f;
                auto dest = juce::Rectangle<float>(img.getWidth() * scale, img.getHeight() * scale)
                              .withCentre(knobRect.getCentre().toFloat());
                g.drawImage(img, dest);
            }
            else
            {
                // Fallback vector knob
                g.setColour(juce::Colour(0xFF4a90e2));
                g.fillEllipse(knobRect.toFloat());
                g.setColour(juce::Colour(0xFF6bb6ff));
                g.drawEllipse(knobRect.toFloat(), 2.0f);
            }
        ...
        if (remainingArea.getHeight() > 100)
        {
            ...
            // Create a 2x3 grid layout for processes - larger cells
            int cols = 2, rows = 3;
            int cellW = (remainingArea.getWidth() - 30) / cols;
            int cellH = (remainingArea.getHeight() - 20) / rows;
            ...
            for (int i = 0; i < 6; ++i)
            {
                ...
                // Neptune small knob (left side) - bigger size
                auto knobArea = cellArea.removeFromLeft(85).reduced(5); // bigger knob area
                
                // Neptune small knob with proper frame selection - larger scale
                if (! skin.knobSmallFrames.empty())
                {
                    int frameIdx = (i * 21) % (int)skin.knobSmallFrames.size(); // different spacing for variety
                    auto img = skin.knobSmallFrames[frameIdx];
                    float scale = juce::jmin(knobArea.getWidth() / (float)img.getWidth(), 
                                           knobArea.getHeight() / (float)img.getHeight()) * 1.1f; // bigger scale
                    auto dest = juce::Rectangle<float>(img.getWidth() * scale, img.getHeight() * scale)
                                  .withCentre(knobArea.getCentre().toFloat());
                    g.drawImage(img, dest);
                }
                // Fallback to big knobs if small knobs not available
                else if (! skin.knobFrames.empty())
                {
                    int frameIdx = (i * 43) % (int)skin.knobFrames.size();
                    auto img = skin.knobFrames[frameIdx];
                    float scale = juce::jmin(knobArea.getWidth() / (float)img.getWidth(), 
                                           knobArea.getHeight() / (float)img.getHeight()) * 0.9f; // bigger scale
                    auto dest = juce::Rectangle<float>(img.getWidth() * scale, img.getHeight() * scale)
                                  .withCentre(knobArea.getCentre().toFloat());
                    g.drawImage(img, dest);
                }
                
                // Neptune button (right side) - proper sizing
                auto btnArea = cellArea.removeFromRight(110).reduced(3, 8); // reasonable margins
                
                // Use Neptune button if available
                if (! skin.buttonFrames.empty() && skin.buttonFrames.size() >= 2)
                {
                    auto img = skin.buttonFrames[0]; // use off state
                    ...
                    // Button text over Neptune button - smaller font
                    g.setColour(juce::Colours::white);
                ...
                g.drawText(details[i], detailRect, juce::Justification::topLeft, false); // false = no wrapping
            }
        }
    }
...
void UIDemoComponent::resized()
{
    ...
    auto transportArea = workArea.removeFromTop(25); // smaller transport size
    
    // Reserve space for Mix knob above Professional Mojo Controls
    auto mixKnobArea = workArea.removeFromTop(120); // 120px height for large mix knob
    workArea.removeFromTop(2); // very minimal gap between knob and Mojo window
    ...
    if (leftAreaWithMargin.getWidth() > 0 && leftAreaWithMargin.getHeight() > 200)
    {
        ...
        // Center the Input label under the meter
        auto centeredLabelArea = labelArea.withWidth(50).withX(leftAreaWithMargin.getX() + 10); // same X as meter
        ...
        if (ledMeter1) {
            auto meterRect = meterArea.withWidth(50).withX(leftAreaWithMargin.getX() + 10);
            ledMeter1->setBounds(meterRect);
        }
        
        // Slider below meter - responsive sizing, closer to meter
        auto sliderRect = leftSliderArea.withWidth(60).withX(leftAreaWithMargin.getX() + 5);
