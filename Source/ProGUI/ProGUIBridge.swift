...
@objc public class ProGUIBridge: NSObject {
    ...
    public init(type: ProGUIComponentType, frame: CGRect) {
        ...
        // Preferred: native-view subviewing; Fallback: attach-to-parent
        if let component = componentPtr {
            if let nativePtr = ProGUI_GetNativeView(component) {
                let nsView = Unmanaged<NSView>.fromOpaque(nativePtr).takeUnretainedValue()
                nsView.frame = self.view.bounds
                nsView.autoresizingMask = [.width, .height]
                self.view.addSubview(nsView)
                NSLog("[Bridge] Added native JUCE view as subview; subviews=\(self.view.subviews.count)")
                logger.info("[ProGUIBridge] Added native JUCE view as subview. Container subviews: \(self.view.subviews.count)")
            } else {
                NSLog("[Bridge] Native view nil; falling back to attach-to-parent")
                logger.error("[ProGUIBridge] ERROR: ProGUI_GetNativeView returned nil; using attach-to-parent")
                let parentPtr = Unmanaged.passUnretained(self.view).toOpaque()
                ProGUI_AttachToParent(component, parentPtr)
                NSLog("[Bridge] Called ProGUI_AttachToParent")
            }
        } else {
