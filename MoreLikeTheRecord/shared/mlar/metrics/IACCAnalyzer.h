 ...
 * - Provides IACC timeline for stereo image stability visualization
 * - Computes IACC variance for stereo stability assessment
 */
class IACCAnalyzer {
public:
     ...
     * @param hopSize Hop size between frames (for overlap)
     */
    ...
    void compute(const juce::AudioBuffer<float>& buffer, std::vector<float>& timeline) {
        ...
        if (iaccHistory.size() > maxHistory) {
            iaccHistory.erase(iaccHistory.begin(), 
                            iaccHistory.begin() + (iaccHistory.size() - maxHistory));
        }
    }
     ...
     * 
     * @return Most recent IACC value (-1.0 to 1.0)
     */
     ...
     * 
     * @return Variance of recent IACC values
     */
...
private:
    /**
     * Process a complete frame of audio to calculate IACC
     */
    void processFrame() {
        // Calculate IACC
        float sumLR = 0.0f;
