 ...
 * - Rise time differences (µs)
 * - Pre-ring energy differences (dB)
...
public:
    ...
    void analyze(const juce::AudioBuffer<float>& pre, const juce::AudioBuffer<float>& post) {
        ...
        const float sampleRate = 48000.0f; // Default if unknown
        const float microSecondsPerSample = 1000000.0f / sampleRate;
        riseDeltaUs = (postRiseTime - preRiseTime) * microSecondsPerSample;
